...
const WishlistFilter = ({
    ...
    const handleTenantTypeClick = (type) => {
        setSelectedTenantTypes((prev) => {
            ...
            const filters = {
                tenant_type: newTypes.length ? newTypes.join(",") : undefined,
    ...
    const applyFilters = (newFilters) => {
        ...
        router.get("/wishlist", cleanFilters, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
            only: ["wishlists", "productTypes", "starCount"],
            onSuccess: () => {
                if (onFilterChange) {
    ...
    useEffect(() => {
        ...
        if (params.get("food_type")) {
            setSelectedFoodTypes(params.get("food_type").split(","));
        }

        if (params.get("tenant_type")) {
            setSelectedTenantTypes(params.get("tenant_type").split(","));
        }

        if (params.get("min_price") || params.get("max_price")) {
            const min = parseInt(params.get("min_price")) || 30000;
            const max = parseInt(params.get("max_price")) || 100000;
            setPriceRange({
                min_price: min,
                max_price: max,
            });
        ...
        if (params.get("rating")) {
            setSelectedRatings(params.get("rating").split(",").map(Number));
        }
    }, []);
