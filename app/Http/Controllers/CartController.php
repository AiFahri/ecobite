...
class CartController extends Controller
{
    public function index()
    {
        ...
        if ($user) {
            ...
            $cartItems = $cartItems->map(function ($item) use ($user) {
                return [
                    'id' => $item->id,
                    'product' => [
                        'id' => $item->product->id,
                        ...
                        'image' => $item->product->productMedia->first()?->photo_url,
                        'tenant' => [
                            'name' => $item->product->tenant->name,
                        ],
                        'isWishlisted' => $item->product->wishlists()
                            ->where('user_id', $user->id)
                            ->exists(),
                    ],
